...
current_selection = 0
window_options = ["Help", "Sensetive Thresholds", "Finger Info"]
...
def display_help_window():
    ...
    draw_text(help_image, '5. Press \'C\' to turn on window controller', (10, 235), color=(255, 0, 0))
    draw_text(help_image, '6. Press \'[\' or \']\' to adjust Metronome interval', (10, 260), color=(130, 0, 75))
...
def display_remote_control():
    remote_image = np.full((300, 300, 3), (0, 0, 0), dtype=np.uint8)
    y_pos = 30
    for i, option in enumerate(window_options):
        ...
        y_pos += 40
    draw_text(remote_image, 'Press Enter to toggle', (10, 240), font_scale=0.6, color=(255, 255, 255), shadow_color=(0, 0, 0))
    draw_text(remote_image, 'Press \'Z/X\' to select', (10, 260), font_scale=0.6, color=(255, 255, 255), shadow_color=(0, 0, 0)) 
    draw_text(remote_image, 'Press \'C\' to turn off', (10, 280), font_scale=0.6, color=(255, 255, 255), shadow_color=(0, 0, 0))
    return remote_image
...
def window_controller(sound_manager, angles):
    ...
    if threshold_window_visible:
        ...
        cv.imshow('Thresholds', threshold_image)
    if remote_control_visible:
        remote_image = display_remote_control()
        cv.imshow('Remote Control', remote_image)
